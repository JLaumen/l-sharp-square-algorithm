...
def run_test_case(filename: str, solver_timeout, replace_basis, use_compatibility, horizon: int | None = None,
                  example: bool = False) -> dict[str, Any]:
    alphabet = [True, False]
...
def run_test_cases_pool(file: str, extension: str, solver_timeout, replace_basis, use_compatibility,
    ...
    with open(f"benchmarking/results/benchmark2{extension}_{file}.csv", "w") as f:
        ...
        if jobs is None:
            for file_name in file_names:
                row = process_file(file_name, target_folder, solver_timeout, replace_basis, use_compatibility)
                f.write(row)
        else:
...
def main(solver_timeout: int = 200, replace_basis: bool = False, use_compatibility: bool = False,
         jobs: int | None = None, example: int = None) -> None:
    if example:
        if not 4 <= example <= 23:
            print("Example must be between 4 and 23")
            return
        print(f"Running example test case randm{'0' if example < 10 else ''}{example}.02.02.18.020_0030.04.aba.beta\n")
        info = run_test_case(f"all/randm{'0' if example < 10 else ''}{example}.02.02.18.020_0030.04.aba.beta",
                             solver_timeout, replace_basis, use_compatibility, example=True)
        print("Total time:", info['total_time'])
        print("Membership queries:", info['queries_learning'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--timeout", type=int, default=200, help="Solver timeout in seconds. Default: 200")
    ...
    parser.add_argument("-j", "--jobs", type=int, default=None,
                        help="Number of parallel processes. Default: run sequentially")
    parser.add_argument("-e", "--example", type=int, default=None,
                        help="Run example test case instead of benchmarks. Must be between 4 and 23. Default: None (run benchmarks)")
    args = parser.parse_args()

    main(solver_timeout=args.timeout, replace_basis=args.replace_basis, use_compatibility=args.use_compatibility,
         jobs=args.jobs, example=args.example)
