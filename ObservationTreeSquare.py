...
class ObservationTreeSquare:
    ...
    def _promote_node_to_basis(self) -> bool:
        ...
        logger.debug("Basis updated: %d -> %d states (DFA bound %d -> %d).", old_basis_size, len(new_basis),
            old_state_bound, self.size, )
    ...
    def _find_hypothesis(self, ) -> tuple[list[list[int]], list[bool]] | None:
        ...
        try:
            ...
            logger.debug("Starting SAT solve: states=%d, basis=%d, nodes=%d, "
                         "edges=%d, variables=%d, clauses=%d.", number_of_states, basis_size, len(nodes), len(edges),
                allocator.next_var - 1, len(clauses), )
            ...
            if solve_result is not True:
                raise RuntimeError(f"Unexpected CaDiCaL result: {solve_result!r}.")
            ...
            logger.debug("Finished SAT solve: result=%s, elapsed=%.3fs.", solve_result, elapsed, )
    ...
    def build_hypothesis(self) -> Dfa | None:
        ...
        if hypothesis_mapping is None:
            ...
            logger.debug("No SAT hypothesis at %d states; increasing bound to %d.", old_size, self.size, )
    ...
    def process_counter_example(self, counterexample: Sequence[Any], output: bool | DCValue, ) -> None:
        ...
        """
        logger.debug("Received counterexample: length=%d, output=%r.", len(counterexample), output, )
