...
class ObservationTreeSquare:
    def __init__(self, alphabet, sul, solver_timeout, replace_basis, use_compatibility):
        ...
        self.root = MooreNode()
        self.root.set_output(False)
        for letter in self.alphabet:
            self.insert_observation([letter], False)
            ...
            if transition_mapping is not None:
                ...
                for state1 in hypothesis.states:
                    for a in self.alphabet:
                        ...
                        if Apartness.compute_witness_in_tree_and_hypothesis_states(self, self.root,
                                                                                   hypothesis.initial_state) is not None:
